...
def make_obs_radar(yaml_data:dict):
    ...
    for i_radar in filenames:
        ...
        date_str = datetime.fromtimestamp(obs_date.timestamp()).strftime("%Y-%m-%d_%H:%M:%S") #2023_09_02_00:00:00
        ...
        vel_texture['data']  = np.where(vel_texture['data'] <0.001 , np.nan, vel_texture['data'] )
        ...
        # perform dealiasing
        dealias_data = pyart.correct.dealias_region_based(radar,vel_field='VEL', gatefilter=gatefilter)
        radar.add_field("corrected_velocity", dealias_data)
        #radar.fields['corrected_velocity'] =  dealias_data

        # reorder the azimuths
        reorder_var = ['Corrected_dBZ','nonprec_mask', 'corrected_velocity', 'vel_texture','lat', 'lon', 'alt']
        for i_var in reorder_var:
            radar = Radar_prep.ordered_az(radar, i_var)

        # remove non-meteorological echo
        radar.fields['Corrected_dBZ_reorder']['data'] = np.where(radar.fields['nonprec_mask_reorder']['data'] == 1,\
                                                                 np.nan, radar.fields['Corrected_dBZ_reorder']['data'])
        radar.fields['corrected_velocity_reorder']['data'] = np.where(radar.fields['nonprec_mask_reorder']['data'] == 1,\
                                                                 np.nan, radar.fields['corrected_velocity_reorder']['data'])
        # remove the dBZ where the texture of velocity is invalid
        radar.fields['Corrected_dBZ_reorder']['data'] = np.where(radar.fields['vel_texture_reorder']['data'] > 3, \
                                                                 np.nan, radar.fields['Corrected_dBZ_reorder']['data'])
        ...
        refl_stack = radar.fields['Corrected_dBZ_reorder']['data'].reshape(-1, 1)
        vel_stack  = radar.fields['corrected_velocity_reorder']['data'].reshape(-1, 1)
        vel_stack  = np.where(abs(vel_stack)<100, vel_stack, -888888.000)
        ...
        for i in range(0,len(LatLonAlt_Before)):
            ...
            if lat_point == 0.0 or lon_point == 0.0:
                    continue
            if refl_stack[i] >= -20: # and abs(vel_stack[i]) < 100: 
        ...
        with open(fileOut, 'a') as f:
            ...
            f.write("{:>5} {:>12}{:8.3f}  {:8.3f}  {:8.1f}  {:>19}{:6}{:6}\n".format(radar_type, i_radar[13:17], radar.longitude['data'][0], \
                            radar.latitude['data'][0], radar.altitude['data'][0],\
                    ...
                    if nlevel_ValidValue[i,j] > 0:
                        ...
                        for k in range(nlevel_ValidValue[i,j]):
                            f.write("   {:12.1f}{:12.3f}{:4}{:12.3f}  {:12.3f}{:4}{:12.3f}\n"\
                                    .format(values[0, k, i, j], \
                                    values[2, k, i, j],vel_qc, vel_err,\
                                    values[1, k, i, j],dbz_qc, dbz_err) )
