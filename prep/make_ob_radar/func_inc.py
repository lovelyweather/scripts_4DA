...
class Radar_prep(object):
    ...
    def ordered_az(self, var):
        ...
        field_dict = {}
        if var in ['dBZ', 'Corrected_dBZ', 'nonprec_mask','rhohv','lat', 'lon', 'alt']:
            level_loop = [0,2,4,5,6,7,8,9,10]
        elif var in ['VEL','corrected_velocity','vel_texture']:
        ...
        for i_loop, isweep in enumerate(level_loop):
            ...
            for i_ray in range(sweep_slice.start, sweep_slice.stop):
                ...
                if var == 'lat':
                    ...
                    if np.ma.is_masked(self.fields[var]['data']):
                        self.fields[var]['data'][np.ma.getmask(self.fields[var]['data'])] = np.nan
